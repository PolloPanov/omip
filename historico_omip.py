...
def extraer_historico():
    """Lee todos los CSV diarios ya guardados y los convierte en histórico."""
    registros = []
    ...
    # Un mismo contrato puede aparecer más de una vez en un archivo diario.
...
def generar_grafica(datos, titulo, ruta):
    ...
    # Evita una leyenda ilegible cuando hay muchos vencimientos.
    if len(contratos) <= 20:
...
def generar_mes(historico, anio, mes):
    datos = historico[
        (historico["fecha"].dt.year == anio)
        & (historico["fecha"].dt.month == mes)
    ]
    ruta = os.path.join("graficas_historicas", f"omip_{anio}_{mes:02d}.png")
    return generar_grafica(
        datos,
        f"Evolución de Precios Futuros OMIP - {MESES[mes - 1].capitalize()} {anio}",
        ruta,
    )
...
def main():
    ...
    parser.add_argument("--30-dias", action="store_true", help="Genera la gráfica de los últimos 30 días disponibles.")
    ...
    if args["30_dias"] if False else False:
        pass

    if args.__contains__("30_dias") and args.__getattribute__("30_dias"):
